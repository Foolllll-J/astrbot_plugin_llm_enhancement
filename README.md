...
一款为 [AstrBot](https://astrbot.app) 深度定制的 LLM 增强插件。通过集成智能唤醒、消息合并以及多媒体解析，显著提升 Bot 在群聊中的参与感、逻辑连贯性和多模态理解能力，让你的 Bot 不再只是被动响应，而是能够“听懂”话题、“看懂”视频、“感知”群成员。
...
* **📦 消息合并增强**：智能聚合短时间内多条消息，支持跨用户上下文合并。
* **🎬 多媒体全量解析**：支持视频与 GIF 抽帧、ASR 语音识别及合并转发内容解析。
...
1. **环境准备**：
   ...
   * 推荐安装 [`astrbot_plugin_blacklist_tools`](https://github.com/Foolllll-J/astrbot_plugin_blacklist_tools) 插件以启用黑名单桥接。
2. **安装 Python 依赖**：
   ```bash
   pip install aiosqlite jieba aiohttp
   ```
3. **插件部署**：
...
* **对话连贯性**：
  * **相关性唤醒**：自动分析当前消息与上下文的关联度。当大家在聊 Bot 感兴趣或擅长的话题时，它会主动跟进。
...
* **撤回感知**：实时监听消息撤回，若首条消息被撤回则自动终止流程，若后续消息被撤回则从合并队列中移除，避免无效 Token 消耗。
* **跨用户合并**：开启后可聚合多人讨论。Bot 会以 `[昵称]: 内容` 的格式清晰分辨发言者，适合处理多人混战。

### 3️⃣ 🎬 理解增强：打破视觉次元壁

* **视频全链路增强**：
  * **多维感知**：自动对视频进行抽帧与 ASR 语音转录，让 Bot 既能“看”又能“听”。
  * **高效抽帧**：采用基于视频时长的动态等距抽帧算法，兼顾解析细腻度与 Token 消耗。
  * **智能屏蔽**：自动识别并屏蔽引用 Bot 自身视频消息的请求（可配置概率），用于忽略用户对非 LLM 消息的回复。
  * **身份注入**：在解析引用消息或转发记录中的多媒体时，自动注入发送者昵称，帮助 LLM 准确分辨“谁发了什么”。
* **合并转发解析**：
  * **多层级解析**：支持完整读取合并转发记录中的文字内容，即使是多层嵌套的记录也能精准提取。
  * **媒体还原**：同步支持解析记录中的视频关键帧与图片，确保 Bot 能够理解完整的聊天上下文。
...
* [astrbot_zssm_explain](https://github.com/xiaoxi68/astrbot_zssm_explain)：提供了视频解析增强、ASR 转录与多媒体注入的实现思路。
