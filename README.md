...
* **特殊信号捕捉**：
  ...
  * **唤醒屏蔽规则**：`wake_forbidden_words` 同样支持普通文本包含、`re:pattern` 与 `/pattern/flags`。命中后将阻止本次唤醒（bot 管理员不受影响）。示例：`re:(政治|黄赌毒)`。
* **情绪与礼节**：
...
* `get_user_avatar`：通过 QQ 头像接口注入头像图片。
* `get_group_members_info`：调用 `get_group_member_list` 获取群成员。
* `set_group_ban`：调用 `set_group_ban` 执行禁言/解禁。
